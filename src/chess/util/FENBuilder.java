...
import static chess.util.Converter.EMPTY_FIELDS;
...
public interface FENBuilder { 
	 ...
	 */
	static Predicate<Field> hasNotMoved = f -> {
	 ...
	 */
	static Predicate<Field> rookOKToCastle = f -> {
	 ...
	 */
	static Predicate<Field> kingOKToCastle = f -> {
		return hasNotMoved.test(f)
	 ...
	 */
	static Function<Game, String> activePlayer = g -> {
	 ...
	 */
	static Function<Game, String> castling = g -> {
	 ...
	 */
	static Function<Game, String> enpassant = g -> {
	 ...
	 */
	static Function<Game, String> count = g -> {
	 ...
	 */
	static Function<Piece, String> pieceToFEN =  p -> { 
		return	FEN_MAP
				.entrySet()
				.stream()
	 ...
	 * 
	 */
	static Function<Game,String> boardToFENEmptyFieldsNotCounted = g -> {
	 ...
	 */
	static Function<Game, StringBuilder> boardToFEN = g -> {
		StringBuilder builder = new StringBuilder();
	 ...
	 */
	static Function<Game, String> gameToFEN = g -> {
	 ...
	 */
	static Function<Game, String> gameToFEN3FoldRule = g -> {
		return boardToFEN.apply(g)
				.append(castling.apply(g))
				.toString(); 
	};
	 ...
	 */
	static void countEmptyFields(StringBuilder builder, int emptyFields) {
		//indexOf changes after delete, need temp to persist correct index for insert.
		int temp; 
		while (builder.indexOf(EMPTY_FIELDS.get(emptyFields)) > 0) { 
			temp = builder.indexOf(EMPTY_FIELDS.get(emptyFields));
			builder.delete(temp, temp + emptyFields);
			builder.insert(temp, emptyFields);
		}
