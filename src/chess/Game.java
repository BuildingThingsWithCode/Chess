...
public class Game {
	...
	/*
	 * returns true if the move results in a check on the opponents king.
	 */
	 ...
	 * necessary piece and sets ObjectProperty<Move> extraMove 
	 ...
	 * castling move for the required rook. And sets ObjectProperty<Move> extraMove.
	 */
		...
		if (m.getPiece() instanceof King && twoColumns.test(m)) {
			if (right.test(m)) {
				move = m.getColor() == WHITE ?
						new Move(getField(0, 0), getField(0, 2)):
	...
	private GameState collectGameState() {
		return new GameState(board.getCopyOfBoardState(), lastMove.getCopy(), getCopyOfTakenPieces());
	}
