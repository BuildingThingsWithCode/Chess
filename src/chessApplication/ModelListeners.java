...
public class ModelListeners {
	...
	public void set() {
		...
		game.getExtraActionRequired().addListener(extraAction = change -> {
			if (change.wasAdded()) {
				boardHandler.executeExtraAction(change.getKey(), change.getValueAdded());
			};
		});
	...
	public void remove() {
		game.getExtraActionRequired().removeListener(extraAction);
